...
{
	/// <summary>
	/// The wrapper for working with memcached
	/// </summary>
	public static class Memcached
	{
		...
		internal static long ExpirationTime
		{
			get
			{
				return Memcached._ExpirationTime;
			}
		}
		#endregion
		...
		public static bool Set(string key, object value, DateTime expiresAt, StoreMode mode = StoreMode.Set)
		{
			return !string.IsNullOrWhiteSpace(key)
				? Memcached.Client.Store(mode, key, value, expiresAt)
				: false;
		}
		...
		{
			return Helper.ExecuteTask<bool>(() => Memcached.Set(key, value, expiresAt, mode), cancellationToken);
		}
		...
		{
			return Memcached.Set(key, value, DateTime.Now.AddMilliseconds(expiresAt), mode);
		}
		...
		{
			return !string.IsNullOrWhiteSpace(key)
				? Memcached.Client.Store(mode, key, value, validFor)
				: false;
		}
		...
		{
			return Helper.ExecuteTask<bool>(() => Memcached.Set(key, value, validFor, mode), cancellationToken);
		}
		...
		{
			return Memcached.Set(key, value, TimeSpan.FromMinutes(expiresAt), mode);
		}
		...
		{
			return Memcached.Set(key, value, TimeSpan.FromMilliseconds(Memcached.ExpirationTime), mode);
		}
		...
		{
			return Helper.ExecuteTask<bool>(() => Memcached.Set(key, value, mode), cancellationToken);
		}
		#endregion
		...
		public static object Get(string key)
		{
			...
			return Memcached.Client.Get(key);
		}
		...
		{
			return Helper.ExecuteTask<object>(() => Memcached.Get(key), cancellationToken);
		}
		...
		public static T Get<T>(string key)
		{
			if (string.IsNullOrWhiteSpace(key))
				throw new ArgumentNullException("key");
			return Memcached.Client.Get<T>(key);
		}
		...
		{
			return Helper.ExecuteTask<T>(() => Memcached.Get<T>(key), cancellationToken);
		}
		...
		public static IDictionary<string, object> Get(IEnumerable<string> keys)
		{
			...
			return Memcached.Client.Get(keys);
		}
		...
		{
			return Helper.ExecuteTask<IDictionary<string, object>>(() => Memcached.Get(keys), cancellationToken);
		}
		#endregion
		...
		public static bool Remove(string key)
		{
			if (string.IsNullOrWhiteSpace(key))
				throw new ArgumentNullException("key");
			return Memcached.Client.Remove(key);
		}
		...
		{
			return Helper.ExecuteTask<bool>(() => Memcached.Remove(key), cancellationToken);
		}
		...
		{
			return Helper.ExecuteTask(() => Memcached.Remove(keys), cancellationToken);
		}
		...
		{
			return Helper.ExecuteTask(() => Memcached.RemoveAll(), cancellationToken);
		}
		#endregion
		...
		{
			if (!Memcached.Client.Append(key, new ArraySegment<byte>(new byte[0])))
			{
				Memcached.Client.Remove(key);
				return false;
			}
		...
		{
			return Helper.ExecuteTask<bool>(() => Memcached.Exists(key), cancellationToken);
		}
		#endregion
