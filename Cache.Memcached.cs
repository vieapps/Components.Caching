...
namespace net.vieapps.Components.Caching
{
	...
	public sealed class Memcached : ICache, IDisposable
	{
			...
			get
			{
				if (Memcached._Client == null)
				{
					...
					else if (ConfigurationManager.GetSection("cache") is CacheConfigurationSectionHandler cacheConfigurationSection)
					{
						...
						throw new ConfigurationErrorsException("No configuration section is found, the configuration file (app.config/web.config) must have a section named 'memcached' or 'cache'.");
				}
		...
		/// <param name="key">The string that presents key of item</param>
		/// <returns>Returns a boolean value indicating if the item is refreshed or not</returns>
		public async Task<bool> RefreshAsync(string key, CancellationToken cancellationToken = default(CancellationToken))
		{
